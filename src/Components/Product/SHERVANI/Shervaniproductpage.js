...
export default function Shervaniproductpage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:3004/data`)
            .then(res => res.json())
            .then(data => {
                const menproduct = findProductInCategory(data, 'Shervani', id);

                const productToSet = menproduct || findProductInCategory(data, 'Shervani', id);
    ...
    const findProductInCategory = (data, category, productId) => {
        ...
        if (categoryContainingProduct) {
            return categoryContainingProduct.Shervanidata.find(product => product.id === parseInt(productId))
        }
        return null;
    };
                ...
                <div className="container my-5">
                    <div className="row details-snippet1">
                        ...
                        <div className="col-md-5 mt-4">
                            <div className="category mb-3"><span className="theme-text">Category:</span>{product.Ctgry}</div>
                            <div className="title">{product.titlemain}</div>
                            <div className="title mt-3" style={{ fontSize: "17px" }}><small>{product.title}</small></div>
                            <div className="ratings my-2">
                                <div className="stars d-flex">
                                    ...
                                    <div className="ml-2">(4.3) 50 Reviews</div>
                                </div>
                            </div>
